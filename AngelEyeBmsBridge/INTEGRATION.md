...
| 啟用 | 是否啟用此端點 |
| 桌台 | 現場顯示用桌名 |
| 來源桌碼 | BMS SourceData 來源桌代碼，例如 `901` |
...
| --- | --- | --- | --- | --- | --- |
| 901桌 | 901 | A8B0E2E1-65F4-4D5D-84C7-6CE30B115101 | SHOE901 | MOXA TCP `10.5.32.24:4001` | 關 |
| 902桌 | 902 | B8B0E2E1-65F4-4D5D-84C7-6CE30B115102 | SHOE902 | MOXA TCP `10.5.32.25:4001` | 關 |
| 903桌 | 903 | C8B0E2E1-65F4-4D5D-84C7-6CE30B115103 | SHOE903 | MOXA TCP `10.5.32.26:4001` | 關 |
...
已存在的 `ANGEL_BAC901`、`ANGEL_BAC902`、`ANGEL_BAC903` 或 `ANGEL_BAC401`、`ANGEL_BAC402`、`ANGEL_BAC403` 設定不會被自動改寫，避免影響既有 BMS 對應；要改用現場 BMS 桌碼時，請在 Bridge 設定中分別改為 `901`、`902`、`903`，或重新建立預設設定。
